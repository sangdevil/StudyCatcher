...
class SSVideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        self.normalize = video_transforms.Compose([
        ...
        if (mode == 'train'):
            ...
            self.heatmap_transform = video_transforms.Compose([
                video_transforms.Resize(self.short_side_size, interpolation='bilinear'),
                video_transforms.CenterCrop(size=(self.crop_size, self.crop_size)),
                volume_transforms.ClipToTensor(channel_nb=self.num_joints)
            ])
        elif mode == 'test':
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            keypoints, _ = self.detect_keypoints(buffer)
            heatmap = self.generate_batch_heatmap(keypoints, height, width)
            ...
            if args.num_sample > 1:
                ...
                for _ in range(args.num_sample):
                    new_frames = self._aug_frame(buffer, heatmap, args)
                ...
                buffer = self._aug_frame(buffer, heatmap, args)
            ...
            heatmap = heatmap.detach().cpu().numpy()
            heatmap = np.transpose(heatmap, (0, 2, 3, 1))
            heatmap = self.data_resize(heatmap)
            ...
            if buffer.shape[1] >= buffer.shape[2]:
                ...
                heatmap = heatmap[temporal_start::2, \
                       spatial_start:spatial_start + self.short_side_size, :, :]
            else:
                buffer = buffer[temporal_start::2, \
                       :, spatial_start:spatial_start + self.short_side_size, :]
    ...
    def _aug_frame(
        ...
        batch_size, channel_dim, _, _ = buffer.shape
        buffer = torch.cat([buffer, heatmap], dim=1)
        # T C H W -> C T H W.
    ...
    def generate_batch_heatmap(self, keypoints, height, width):
        batch_size, _, _ = keypoints.shape
        heatmap = torch.zeros((batch_size, self.num_joints, height, width), dtype=torch.float32)
        ...
        for batch_idx, joints in enumerate(keypoints):
            for joint_idx, pts in enumerate(joints):
                ...
                heatmap[batch_idx, joint_idx, aa:bb, cc:dd] = np.maximum(
                    heatmap[batch_idx, joint_idx, aa:bb, cc:dd], self.g[a:b, c:d]
                )
    ...
    def detect_keypoints(self, image_tensor):
        ...
        # batch_size, _, image_height, image_width = image_tensor.shape
        # images = image_tensor.detach().cpu().numpy()
        # images = np.transpose(images, (0, 2, 3, 1))
        batch_size, image_height, image_width, _ = image_tensor.shape
        mean, std = np.array([0.485, 0.456, 0.406]), np.array([0.229, 0.224, 0.225])

        coords = np.zeros((batch_size, self.num_joints, 3))
        bboxs = torch.zeros((batch_size, 4))
        ...
        with mp_pose.Pose(
            ...
            for img_idx, img in enumerate(image_tensor):
                ...
                # mask = (results.segmentation_mask==1)
                # cols = np.any(mask, axis=0)
                # rows = np.any(mask, axis=1)
                # cmin, cmax = np.where(cols)[0][[0, -1]] # x1,x2
                # rmin, rmax = np.where(rows)[0][[0, -1]] # y1,y2
                # bboxs[img_idx] = torch.tensor([cmin, cmax, rmin, rmax])
