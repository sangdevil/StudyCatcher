...
for i in range(1):
    ...
    checkpoint = torch.load(os.path.join("DreamLadders\checkpoints", f"valid_{i + 2}.pth"), map_location='cpu')
...
def run_classification(frames):
    ...
    print("뭔진 몰라도 완료", time.time() - start_time)

    outputs = []
    with torch.no_grad():
        for model_idx in [0]:
            output = MODELS[model_idx](buffer)
            outputs.append(output.unsqueeze(0))
    print("no grad 어쩌구", time.time() - start_time)
...
while True:
    ...
    if len(frames) < 16:
        frames.append(frame)
    print(time.time() - start_time, len(frames))
